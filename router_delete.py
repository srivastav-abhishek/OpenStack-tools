...
authPort = '9696'
url = "%s://%s:%s/v2.0/routers.json" % (protocol, hostIp, authPort)
#print 'url '+url
...
try:
	...
	if resp.status_code == requests.codes.ok:
	    ...
	    while i!=len(routers["routers"]):
	        router_list.append(str(routers["routers"][i]["id"]))
	        i += 1
...
headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'User-Agent':'python-neutronclient', 'X-Auth-Token':'%s' % token}
url =  'http://10.160.1.3:9696/v2.0/routers/'

try:
    for router in router_list:
	durl = url+router+".json"
	print url
